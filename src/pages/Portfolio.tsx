...
export default function Portfolio() {
  ...
  return (
    <PageTransition>
      ...
      {/* Page Hero Header */}
      <section id="portfolio-hero" className="py-24 md:py-32 border-b border-border bg-card/10">
      ...
      <section id="portfolio-gallery" className="py-16 md:py-24 px-6 lg:px-8 max-w-7xl mx-auto">
        {filteredProjects.length > 0 ? (
            ...
            {filteredProjects.map((project, idx) => {
              ...
              return (
                <ScrollReveal key={project.id} delay={idx * 0.05}>
                  <div 
                    className={cn(
                      "grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center py-10 border-b border-white/5 last:border-b-0",
          ...
          <div className="text-center py-24 border border-dashed border-border rounded-sm bg-accent/20">
            <p className="font-mono text-xs text-muted-foreground tracking-widest uppercase">
              !! ZERO_RECORDS_FOUND_FOR_CRITERIA
            </p>
            <button
              onClick={() => setActiveCategory('all')}
              className="mt-4 inline-flex px-4 py-2 border border-border rounded-sm font-mono text-xs text-foreground hover:bg-accent transition-colors"
            >
              reset.filters()
            </button>
          </div>
